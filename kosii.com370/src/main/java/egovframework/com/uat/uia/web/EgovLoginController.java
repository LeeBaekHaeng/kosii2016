...
public class EgovLoginController {
	...
	public String actionCrtfctLogin(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request,
		 ...
		 * GPKIHttpServletRequest gpkirequest = null; String dn = ""; try{
		 * gpkiresponse = new GPKIHttpServletResponse(response); gpkirequest =
		 * new GPKIHttpServletRequest(request);
		 * gpkiresponse.setRequest(gpkirequest); X509Certificate cert = null;
		 * 
		 ...
		 * gpkirequest.ENVELOP_SIGNDATA || message_type ==
		 * gpkirequest.SIGNED_DATA){ signData = gpkirequest.getSignedData();
		 * if(privatekey_random != null) { privatekey_random =
		 * gpkirequest.getSignerRValue(); } signType =
		 * gpkirequest.getSignType(); } queryString =
		 * gpkirequest.getQueryString(); }catch(Exception e){ return
		 * "cmm/egovError"; }
		 * 
		 * // 2. 업무사용자 테이블에서 dn값으로 사용자의 ID, PW를 조회하여 이를 일반로그인 형태로 인증하도록 함 if (dn
		 * != null && !dn.equals("")) {
		 * 
		 ...
		 * 3-1. spring security 연동 return
		 * "redirect:/j_spring_security_check?j_username=" +
		 * resultVO.getUserSe() + resultVO.getId() + "&j_password=" +
		 * resultVO.getUniqId();
		 * 
		 * }else{ // 3-2. 로그인 정보를 세션에 저장
		 * request.getSession().setAttribute("loginVO", resultVO); return
		 * "redirect:/uat/uia/actionMain.do"; }
		 * 
		 * 
	...
	public String searchPassword(@ModelAttribute("loginVO") LoginVO loginVO, ModelMap model) throws Exception {
		...
		if (loginVO == null || loginVO.getId() == null || loginVO.getId().equals("") && loginVO.getName() == null
				...
				|| loginVO.getPasswordHint().equals("") && loginVO.getPasswordCnsr() == null
				|| loginVO.getPasswordCnsr().equals("") && loginVO.getUserSe() == null
	...
	public void getEncodingData() throws Exception {
		...
		/*
		 * X509Certificate x509Cert = null; byte[] cert = null; String
		 * base64cert = null; try { x509Cert = Disk.readCert(
		 * "/product/jeus/egovProps/gpkisecureweb/certs/SVR1311000011_env.cer");
		 * cert = x509Cert.getCert(); Base64 base64 = new Base64(); base64cert =
		 * base64.encode(cert); log.info("+++ Base64로 변환된 인증서는 : " +
		 * base64cert);
		 * 
	...
	public String gpkiRegistView(HttpServletRequest request, HttpServletResponse response, ModelMap model)
		...
		/*
		 * // 브라우저 이름을 받기위한 처리 String webKind =
		 * EgovClntInfo.getClntWebKind(request); String[] ss =
		 * webKind.split(" "); String browser = ss[1];
		 * model.addAttribute("browser",browser); // -- 여기까지 if
		 * (os.equalsIgnoreCase("x86")) { //Local Host TEST 진행중 } else { if
		 * (browser.equalsIgnoreCase("Explorer")) { GPKIHttpServletResponse
		 * gpkiresponse = null; GPKIHttpServletRequest gpkirequest = null;
		 * 
		 * try { gpkiresponse = new GPKIHttpServletResponse(response);
	...
	public String actionGpkiRegist(HttpServletRequest request, HttpServletResponse response, ModelMap model)
		...
		/*
		 * // -- 여기까지 if (os.equalsIgnoreCase("x86")) { // Local Host TEST 진행중 }
		 * else { if (browser.equalsIgnoreCase("Explorer")) {
		 * GPKIHttpServletResponse gpkiresponse = null; GPKIHttpServletRequest
		 * gpkirequest = null; try { gpkiresponse = new
		 * GPKIHttpServletResponse(response); gpkirequest = new
		 * GPKIHttpServletRequest(request);
		 * gpkiresponse.setRequest(gpkirequest); X509Certificate cert = null;
		 * 
		 * // byte[] signData = null; // byte[] privatekey_random = null; //
		 * String signType = ""; // String queryString = "";
		 * 
		 * cert = gpkirequest.getSignerCert(); dn = cert.getSubjectDN();
		 * 
		 ...
		 * gpkiresponse.getChallenge());
		 * 
		 * return "egovframework/com/uat/uia/EgovGpkiRegist"; } catch (Exception
		 * e) { return "egovframework/com/cmm/egovError"; } } }
		 */
		return "egovframework/com/uat/uia/EgovGpkiRegist";
	}
}